...
namespace WhisperSubs.Controller
{
    public class SubtitleManager
    {
            ...
            List<string> resolvedLanguages, bool force, CancellationToken cancellationToken)
        {
            ...
            // Issue #44: warn (once per item, after the skip guards above) when translating with a
            // turbo model. whisper.cpp's distilled turbo models were fine-tuned WITHOUT the translate
            // task, so --translate silently emits the source language instead of English. We don't
            // block — behavior is unchanged — but this surfaces what was previously a silent failure.
