...
- **Desired-languages allow-list + image-subs toggle (issue #83)**: as of this version, `DesiredSubtitleLanguages` (parsed by `SubtitleInventory.ParseDesiredLanguages` into a normalized 639-1 set, checked via `SubtitleInventory.IsLanguageDesired`) gates the full pass per audio language and the translation pass on `"en"` inside `SubtitleManager.GenerateSubtitleAsync`. Empty set = no filter (pre-#83 behavior, all audio languages + English per `EnableTranslation`); a manual request (`force`) bypasses it so an explicit "Generate" always runs. Forced subs are never language-filtered. `CountImageSubtitlesAsPresent` (default false) flips the `requireText` arg of `HasUsableSubtitle`/`IsUsableStream` (`requireText: !CountImageSubtitlesAsPresent`) in `SubtitleManager.ShouldSkipForExistingSubtitle` and `SubtitleGenerationTask` — when on, image subs (PGS/VOBSUB) count as already-present and no text subtitle is generated.
