...
- **Generate-which-languages toggles + image-subs toggle (issue #83)**: whisper can only produce a subtitle in the audio's own language (transcribe) or English (translate — English is the only translate target), so the UI offers exactly two checkboxes, both default true: `GenerateOriginalLanguageSubtitles` and `GenerateEnglishSubtitles`. In `SubtitleManager.GenerateSubtitleAsync` these become `wantOriginal`/`wantEnglish` (a manual `force` request sets both true). The full pass runs for a language when `wantOriginal` OR (the audio is English AND `wantEnglish`); the translation pass runs when `wantEnglish`. Forced subs are governed by `SubtitleMode`, not these toggles. If both are off, nothing is generated and a warning logs. (An earlier free-text `DesiredSubtitleLanguages` allow-list was replaced by these toggles before release — never offer languages whisper can't produce.) `CountImageSubtitlesAsPresent` (default false) flips the `requireText` arg of `HasUsableSubtitle`/`IsUsableStream` (`requireText: !CountImageSubtitlesAsPresent`) in `SubtitleManager.ShouldSkipForExistingSubtitle` and `SubtitleGenerationTask` — when on, image subs (PGS/VOBSUB) count as already-present.
